...
int main(int argc, char **argv) {
    ...
    if (pid == 0){
        ...
        colors = calloc(points*points,sizeof(double)); // массив с цветами для сборки
    }

    int Np = points/nproc;
    printf("%d", Np);
    ylocal = calloc(Np,sizeof(double)); // здесь будут храниться значения y для каждого процесса
    colors_local = calloc(points*Np,sizeof(double)); // локальные массивы для вычислений

    MPI_Scatter(ypoints,Np,MPI_DOUBLE,ylocal,Np,MPI_DOUBLE,0,MPI_COMM_WORLD);
    ...
    MPI_Gather(colors_local,points*Np,MPI_DOUBLE,
                        colors,points*Np,MPI_DOUBLE,0,MPI_COMM_WORLD);
    if (pid == 0){
